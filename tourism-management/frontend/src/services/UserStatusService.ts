 ...
 */
export const updateOnlineStatus = async (userId: string, isOnline: boolean): Promise<UserStatus> => {
 ...
 * Call this periodically (e.g., every 2-3 minutes) when user is active
 */
...
/**
 * Mark inactive users as offline (admin only)
 */
export const cleanupInactiveUsers = async (): Promise<number> => {
  const { data } = await apiClient.post('/auth/status/cleanup');
  return data.users_marked_offline;
};

/**
 * Hook to automatically send heartbeat
 * Usage: Add to your main App component or layout
 */
export const useHeartbeat = (intervalMinutes: number = 3) => {
