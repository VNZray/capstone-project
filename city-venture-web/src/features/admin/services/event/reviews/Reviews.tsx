...
import type { EventReview } from "@/src/types/Event";

const transformReview = (apiReview: EventReview): Review => {
  const fullName = apiReview.user_first_name || apiReview.user_last_name
    ? `${apiReview.user_first_name || ""} ${apiReview.user_last_name || ""}`.trim()
    : "Anonymous";
  ...
  return {
    ...
    user: {
      name: fullName,
      avatar: undefined,
      verified: apiReview.is_verified_attendee,
    },
    rating: apiReview.rating as 1 | 2 | 3 | 4 | 5,
    createdAt: apiReview.created_at,
    text: apiReview.review_text || "",
    images: apiReview.photos?.map((p) => p.file_url) || [],
    reply: undefined, // Event reviews don't have replies yet
  };
};
...
const EventReviews: React.FC = () => {
  ...
  const fetchReviews = React.useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      // Fetch all reviews (not just approved) for admin view
      const apiReviews = await apiService.getEventReviews(id, { status: "approved" });
  ...
  const handleSaveReply = async (reviewId: string, text: string) => {
    // TODO: Implement reply functionality for event reviews
    console.log("Save reply:", reviewId, text);
    alert("Reply functionality coming soon!");
  };

  const handleDeleteReply = async (reviewId: string) => {
    // TODO: Implement delete reply functionality
    console.log("Delete reply:", reviewId);
  };
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {filtered.length === 0 && (
                <NoDataFound
                  ...
                  message="Once attendees start leaving feedback, you can view & respond here."
                />
              )}
