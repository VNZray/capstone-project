...
const Accommodation: React.FC = () => {
  ...
  const tabs = [
    ...
    ...Array.from(
      new Set(
        accommodations.flatMap(
          (acc) => acc.categories?.map((c) => c.category_id) || []
        )
      )
    )
  ...
  const fetchAccommodations = async () => {
    ...
    try {
      ...
      const accommodationData = Array.isArray(response)
        ? response.filter(
            (business: BusinessDetails) => business.hasBooking === true
          )
        : [];
  ...
  const filteredAccommodations = accommodations.filter((acc) => {
    ...
    const matchesCategory =
      activeTab === "all" ||
      acc.categories?.some((c) => c.category_id === parseInt(activeTab)) ||
      false;
