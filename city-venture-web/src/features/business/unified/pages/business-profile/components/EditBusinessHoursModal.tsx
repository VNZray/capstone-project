...
import { updateData, insertData } from "@/src/services/Service";
...
const EditBusinessHoursModal: React.FC<EditBusinessHoursModalProps> = ({
  ...
  const init =
    initialBusinessHours && initialBusinessHours.length > 0
      ? initialBusinessHours
      : defaultWeek.map(
          (d) =>
            ({
    ...
    try {
      if (businessId) {
        ...
        for (const h of hours) {
          ...
          const hasValidId =
            h.id !== undefined && h.id !== null && String(h.id).trim() !== "";
          if (hasValidId) {
            // update existing row
            await updateData(
              String(h.id),
