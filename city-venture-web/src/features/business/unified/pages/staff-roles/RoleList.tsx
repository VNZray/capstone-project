 ...
 * RoleList Component
 * 
 * Displays a list of business roles with actions for editing and deleting.
 * Shows role details including permission count and user count.
 */

import { Box, Card, CardContent, Chip, IconButton, Stack, Tooltip, Typography } from '@mui/joy';
import { Edit, Trash2, Users, Shield } from 'lucide-react';
import type { Role } from './types';
import { canEditRole, canDeleteRole, getRoleTypeLabel, getRoleTypeColor } from '@/src/services/RoleService';
...
export function RoleList({ roles, onEdit, onDelete, onViewDetails, isLoading }: RoleListProps) {
  if (isLoading) {
    return (
      <Stack spacing={2}>
        {[1, 2, 3].map((i) => (
          <Card key={i} variant="outlined" sx={{ opacity: 0.5 }}>
            <CardContent>
              <Box sx={{ height: 60, bgcolor: 'neutral.100', borderRadius: 'sm' }} />
            </CardContent>
          </Card>
        ))}
      </Stack>
  ...
  return (
    <Card
      ...
      sx={{
        ...
        '&:hover': onViewDetails ? { borderColor: 'primary.300', bgcolor: 'primary.50' } : {},
        transition: 'all 0.2s',
      }}
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box sx={{ flex: 1 }}>
            <Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
              ...
              >
                {role.is_custom ? 'Custom' : role.based_on_name ? `From ${role.based_on_name}` : getRoleTypeLabel(role.role_type)}
              </Chip>
            </Stack>
