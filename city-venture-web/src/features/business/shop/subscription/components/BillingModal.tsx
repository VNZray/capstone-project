  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ModalDialog sx={{ maxWidth: 600, width: "90%", maxHeight: "90vh", overflow: "auto" }}>
        ...
        <Box sx={{ mb: 2 }}>
          <Typography.Header size="sm">Complete Your Upgrade</Typography.Header>
          <Typography.Body size="sm" sx={{ mt: 1, color: "text.secondary" }}>
        ...
        <Container elevation={0} padding="16px" gap="12px" background="#F9FAFB">
          <Typography.Label size="sm">Order Summary</Typography.Label>
          
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Typography.Body size="sm">{planName} Plan</Typography.Body>
            <Typography.Body size="sm" weight="bold">
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Typography.Label size="sm">Total</Typography.Label>
          ...
          <RadioGroup value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethod)}>
            <Stack spacing={1}>
              <Container
                ...
                cursor="pointer"
                style={{
                  border: paymentMethod === "gcash" ? "2px solid #3B82F6" : "1px solid #E5E7EB",
                }}
              >
                <Radio value="gcash" />
                <Smartphone size={24} color="#007DFF" />
              ...
              <Container
                ...
                cursor="pointer"
                style={{
                  border: paymentMethod === "maya" ? "2px solid #3B82F6" : "1px solid #E5E7EB",
                }}
              >
                <Radio value="maya" />
                <Smartphone size={24} color="#00D632" />
                <Typography.Body size="sm" weight="semibold">
                  Maya
                </Typography.Body>
              </Container>
        ...
        <Box sx={{ mt: 2 }}>
          {paymentMethod === "credit" ? (
            ...
            <Stack spacing={2}>
              ...
              <Container elevation={0} padding="12px" gap="8px" background="#FEF3C7">
                <Box sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
                  <AlertCircle size={16} color="#F59E0B" style={{ flexShrink: 0, marginTop: 2 }} />
                  <Typography.Body size="xs">
                    You will be redirected to {paymentMethod === "gcash" ? "GCash" : "Maya"} app to complete the
                    payment. Please ensure your account has sufficient balance.
                  </Typography.Body>
                </Box>
              </Container>
            </Stack>
        ...
        <Box sx={{ display: "flex", gap: 2, mt: 3 }}>
          <Button variant="outlined" colorScheme="secondary" fullWidth onClick={onClose} disabled={processing}>
            Cancel
          </Button>
          <Button
            variant="solid"
          ...
          >
            {processing ? "Processing..." : `Pay ₱${planPrice.toLocaleString()}`}
          </Button>
        </Box>
      </ModalDialog>
    </Modal>
