...
export async function seed(knex) {
  ...
  // Create an owner linked to owner user (address optional)
  // We'll create a simple address for demonstration: Naga City, Barangay Abella
  const [addressId] = await knex('address').insert({ province_id: 20, municipality_id: 24, barangay_id: 1 }).returning('id');
  const resolvedAddressId = typeof addressId === 'object' && addressId.id ? addressId.id : addressId; // compatibility for mysql returning
