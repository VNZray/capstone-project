...
const DropdownInner = <T extends DropdownItem = DropdownItem>(
  ...
  const [selectedId, setSelectedId] = useState<string | number | null>(defaultValue);
  const [selectedIds, setSelectedIds] = useState<Array<string | number>>(defaultValues);
  ...
  const effectiveSelectedIds = controlledMulti ? (values as Array<string | number>) : selectedIds;
  ...
  const sizeConfig = useMemo(() => {
    switch (size) {
      case 'small': return { height: 40, fontSize: 13, padding: 10, icon: 14 };
      case 'large': return { height: 54, fontSize: 16, padding: 16, icon: 18 };
      case 'medium':
      default: return { height: 48, fontSize: 14, padding: 14, icon: 16 };
    }
  }, [size]);
  ...
  }, [multi, effectiveSelectedIds, internalItems, effectiveSelectedId, placeholder]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    getSelected: () => {
      if (multi) {
        return internalItems.filter(i => effectiveSelectedIds.includes(i.id));
      }
      const single = internalItems.find(i => i.id === effectiveSelectedId) || null;
      return single;
    },
    refetch: async () => { await loadItems(); },
  }), [multi, internalItems, effectiveSelectedIds, effectiveSelectedId, loadItems]);

  // trigger styles
  const triggerVariant = getTriggerVariantStyles(variant, surface, borderColor, accent, isDark);
  ...
  <View style={style} testID={testID}>
      ...
      <Pressable
        ...
        disabled={disabled}
        ref={triggerRef}
        onPress={() => { if (!disabled) { Keyboard.dismiss(); setOpen(o => !o); if (!open && fetchItems && !internalItems.length) loadItems(); } }}
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          {clearable && ((multi && effectiveSelectedIds.length) || (!multi && effectiveSelectedId != null)) && (
            <Pressable
              accessibilityLabel="Clear selection"
              hitSlop={10}
              onPress={(e) => { e.stopPropagation(); handleClear(); }}
              style={({ pressed }) => [{ padding: 4, borderRadius: 4 }, pressed && { opacity: 0.6 }]}
            >
              <FontAwesome5 name="times-circle" color={subTextColor} size={16} />
            </Pressable>
          )}
          <FontAwesome5 name={open ? 'chevron-up' : 'chevron-down'} size={16} color={subTextColor} />
        </View>
      </Pressable>

      {!!helperText && !errorText && (
        <ThemedText type="label-extra-small" mt={4} style={{ color: subTextColor }}>{helperText}</ThemedText>
      )}
      {!!errorText && (
        <ThemedText type="label-extra-small" mt={4} style={{ color: colors.error }}>{errorText}</ThemedText>
      )}
      ...
      >
        ...
        <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
          ...
          {(withSearch ?? searchable) && (
            <View style={[styles.searchWrap, { borderColor: borderColor, backgroundColor: isDark ? '#1E2535' : '#F3F6FB', marginTop: 10 }]}>                
              ...
              <TextInput
                ...
                style={{ flex: 1, paddingVertical: 12, color: textColor, fontSize: 13 }}
                autoCorrect={false}
                autoCapitalize="none"
              />
              {search.length > 0 && (
                <Pressable onPress={() => setSearch('')} hitSlop={8}>
                  <FontAwesome5 name="times-circle" size={14} color={subTextColor} />
                </Pressable>
              )}
            </View>
          )}
            ...
            <View style={styles.loadingWrap}>
              ...
              <Text style={{ marginTop: 8, fontSize: 12, color: subTextColor }}>{loadingText}</Text>
            </View>
          ) : (
            <FlatList
              data={filteredItems}
                ...
                return (
                  ...
                  >
                    <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, gap: 10 }}>
                      {item.icon && <FontAwesome5 name={item.icon} size={16} color={accent} />}
                      <View style={{ flex: 1 }}>
                        <ThemedText type='body-medium' numberOfLines={1}>{item.label}</ThemedText>
                        {!!item.description && (
                          <ThemedText type='body-small' style={{ color: subTextColor, marginTop: 2 }} numberOfLines={2}>{item.description}</ThemedText>
                        )}
                      </View>
                    </View>
                    <View style={{ marginLeft: 10 }}>
                      {multi ? (
                        selected ? (
                          <FontAwesome5 name="check-square" size={16} color={accent} />
                        ) : (
                          <FontAwesome5 name="square" size={16} color={subTextColor} />
                        )
                      ) : selected ? (
            ...
            <View style={[styles.footer, { borderColor: borderColor }]}>                
              ...
              <Button
                ...
                size="small"
                label="Apply"
                onPress={() => { setOpen(false); onChangeMulti?.(internalItems.filter(i => effectiveSelectedIds.includes(i.id)) as T[]); }}
              />
            </View>
            )}
          </View>
...
// Forward ref wrapper ------------------------------------------------
const Dropdown = React.forwardRef(DropdownInner) as <T extends DropdownItem = DropdownItem>(
