 ...
 * Seeds initial Privacy Policy and Terms and Conditions content
 *
 * Run: npx knex seed:run --specific=13_app_legal_policies.cjs
 */
...
exports.seed = async function (knex) {
  ...
  await knex('app_legal_policies').del();
...
*By using City Venture, you acknowledge that you have read, understood, and agree to be bound by these Terms and Conditions.*`;
  ...
  // Insert the legal policies
  await knex('app_legal_policies').insert({
    id: knex.raw('(UUID())'),
  ...
  console.log('✅ App legal policies seeded successfully');
};
