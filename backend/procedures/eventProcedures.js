async function createEventProcedures(knex) {
  ...
  await knex.raw(`
    CREATE PROCEDURE GetEventCategoryById(IN p_id CHAR(64))
    BEGIN
      SELECT id, name, description, icon, is_active, created_at, updated_at 
      FROM event_categories 
      WHERE id = p_id;
  ...
  await knex.raw(`
    CREATE PROCEDURE DeleteEventCategory(IN p_id CHAR(64))
    BEGIN
  ...
  await knex.raw(`
    CREATE PROCEDURE GetEventCategoryMapping(IN p_event_id CHAR(64))
    BEGIN
  ...
  // Get event locations
  await knex.raw(`
    CREATE PROCEDURE GetEventLocations(IN p_event_id CHAR(64))
    BEGIN
      SELECT 
        el.*,
        b.barangay as barangay_name,
  ...
  await knex.raw(`
    ...
    BEGIN
      ...
      INSERT INTO events (
        ...
        p_name, p_description, p_category_id, p_venue_name, p_venue_address, p_barangay_id,
  ...
  // Unfeature event
  await knex.raw(`
    CREATE PROCEDURE UnfeatureEvent(IN p_id CHAR(64))
    BEGIN
      UPDATE events 
  ...
  await knex.raw(`
    CREATE PROCEDURE ClearEventCategoryMappings(IN p_event_id CHAR(64))
    BEGIN
  ...
  await knex.raw(`
    ...
    BEGIN
      ...
      VALUES (p_event_id, p_venue_name, p_venue_address, p_barangay_id, p_latitude, p_longitude, p_is_primary, p_display_order);
...
export { createEventProcedures, dropEventProcedures };
