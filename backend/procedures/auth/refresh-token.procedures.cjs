async function createRefreshTokenProcedures(knex) {
  // Note: GetUserByEmail is created by staff-merged.procedures.js (migration 20251021043733_staff_table.cjs)
  // which runs before this migration, so we don't recreate it here
...
async function dropRefreshTokenProcedures(knex) {
  // Note: Don't drop GetUserByEmail here - it's owned by staff-merged.procedures.js
...
module.exports = { createRefreshTokenProcedures, dropRefreshTokenProcedures };
