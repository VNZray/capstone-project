...
const { createRefundProcedures, dropRefundProcedures } = require("../procedures/refundProcedures.cjs");
...
exports.up = async function (knex) {
  ...
  // Add refund-related fields to order table
  await knex.schema.alterTable("order", (table) => {
    // Add refund_requested status if not exists
    // Note: This is handled by existing ENUM, just add tracking fields
...
exports.down = async function (knex) {
  ...
  console.log("Dropping refund stored procedures...");
  try {
  ...
  // Remove refund-related columns from order table
  await knex.schema.alterTable("order", (table) => {
